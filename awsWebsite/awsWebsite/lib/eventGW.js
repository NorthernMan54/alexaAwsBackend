...
async function send(user, message, callback) {
  ...
  try {
    ...
    await sendEvent(message, user, token);
  } catch (err) {
    console.log("eventGW Error sending event:", user, token, err.toString());
    measurement.send({
      t: 'event',
      ec: 'event',
      ea: err.toString(),
...
function sendEvent(message, user, token) {
  return new Promise((resolve, reject) => {
    ...
    request({
      ...
      if (!err && response.statusCode === 202) {
        ...
        resolve("Event Gateway Response Code: " + response.statusCode);
      } else {
        delete cache[user];
        reject(err);
      }
